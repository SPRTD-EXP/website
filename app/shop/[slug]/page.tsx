...
export default function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!product) return <main className="min-h-screen bg-background"><Navbar /></main>;
  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <div className="pt-2 grid grid-cols-1 lg:grid-cols-[3fr_2fr] items-start">
        ...
        <div className="border-r border-border">
          {imageUrls.length > 0 ? (
            imageUrls.map((url, i) => (
              <div
                key={url}
                className={`relative w-full aspect-[3/4]${i < imageUrls.length - 1 ? ' border-b border-border' : ''}`}
              >
        ...
        <div className="product-info flex flex-col px-6 pt-8 pb-12 md:px-12 md:pt-16 md:pb-16 border-t border-border lg:border-t-0 lg:sticky lg:top-14 lg:h-[calc(100vh-3.5rem)] lg:overflow-y-auto">

          {/* Breadcrumb */}
          <Link
            href="/shop"
            className="text-foreground hover:text-gold transition-colors uppercase tracking-[0.22em] mb-10 self-start"
          ...
          >
            COLOR — <span className="text-foreground">{selectedColorway.toUpperCase()}</span>
          </p>
          ...
          <p
            className="mb-4 uppercase tracking-[0.22em]"
            style={{ fontWeight: 300, fontSize: '9px', color: sizeError ? '#ff6b6b' : '#f5f0e8' }}
          >
          ...
          <div className="mt-10 flex flex-col border-t border-border">
            {['SHIPPING & RETURNS', 'SIZE GUIDE', 'CONTACT'].map(label => (
              ...
              >
                <span
                  className="text-foreground group-hover:text-gold transition-colors uppercase tracking-[0.18em]"
                  style={{ fontWeight: 300, fontSize: '9px' }}
                >
                  {label}
                </span>
                <span className="text-foreground group-hover:text-gold transition-colors text-xs">›</span>
              </div>
            ))}
          </div>
        </div>
